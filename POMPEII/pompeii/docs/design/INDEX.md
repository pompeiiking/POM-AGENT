...
- **开发状态与接口（深度分析）**：`开发状态与系统接口.md` — 进度、功能流、完成度、未完成模块、对外接口、可替换性
- **会话存储与归档实现说明**：`会话存储与归档实现说明.md` — 热数据与 SQLite 双表、`ephemeral` 与生产路径、未实现项

- **架构设计**：
  - `架构设计ver0.4.md`：整体微内核架构与分层说明（当前有效版本）
  - `现有结构与调用链图.md`：当前代码结构与调用链路的可视化
  - `archive/`：历史草案 `架构设计ver0.2.md`、`架构设计ver0.3.md`，以及早期随笔（非正式架构文档）
